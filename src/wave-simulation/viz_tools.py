...
def anim_1D(x, y, pas_de_temps, pas_d_images, save=False, myxlim=(0, 4), myylim=(-4, 4)):
    ...
    def animate(i):
        ...
        ax.set_title("$u(x)$ à t = {} s".format(
            np.round(i*pas_d_images*pas_de_temps, 4)), fontname="serif", fontsize=16)
        return line,

    # call the animator.  blit=True means only re-draw the parts that have changed.
    anim = animation.FuncAnimation(
        fig, animate, init_func=init, frames=y.shape[1]//pas_d_images, interval=10, blit=True)
...
def anim_2D(X, Y, L, pas_de_temps, pas_d_images, save=False, myzlim=(-0.15, 0.15)):
    ...
    def update_surf(num):
        ...
        surf = ax.plot_surface(
            SX, SY, L[:, :, pas_d_images*num], cmap=plt.cm.viridis)
        ax.set_xlabel("x [m]", fontname="serif", fontsize=14)
        ax.set_ylabel("y [m]", fontname="serif", fontsize=14)
        ax.set_zlabel("$u$ [m]", fontname="serif", fontsize=16)
        ax.set_title("$u(x,y)$ à t = {} s".format(
            np.round(pas_d_images*num*pas_de_temps, 4)), fontname="serif", fontsize=16)
        ax.set_zlim(myzlim[0], myzlim[1])
        plt.tight_layout()
        return surf,
    ...
    # call the animator.  blit=True means only re-draw the parts that have changed.
    anim = animation.FuncAnimation(
        fig, update_surf, frames=L.shape[2]//pas_d_images, interval=50, blit=False)

    # Save the result
    if save:
        writer = animation.FFMpegWriter(
            fps=24, bitrate=10000, codec="libx264", extra_args=["-pix_fmt", "yuv420p"])
...
def anim_2D_flat(X, Y, L, pas_de_temps, pas_d_images, save=False,  myzlim=(-0.15, 0.15)):
    ...
    mesh = plt.pcolormesh(
        X, Y,  L[:, :, 0], vmin=myzlim[0], vmax=myzlim[1], cmap=plt.cm.viridis)
    plt.colorbar(mesh, orientation="vertical")

    # Update function
    def update_surf(num):
        ax.set_title("$u(x,y)$ à t = {} s".format(
            np.round(num*pas_d_images*pas_de_temps, 4)), fontname="serif", fontsize=16)
        mesh.set_array(L[:, :, pas_d_images*num][:-1, :-1].flatten())
        return mesh,

    anim = animation.FuncAnimation(
        fig, update_surf, frames=L.shape[2]//pas_d_images, interval=10, blit=False)
    # Save the result
    if save:
        writer = animation.FFMpegWriter(
            fps=24, bitrate=10000, codec="libx264", extra_args=["-pix_fmt", "yuv420p"])
